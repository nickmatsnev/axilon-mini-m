...
class _ChatPageState extends State<ChatPage> with TickerProviderStateMixin {
  ...
  void _sendTextMessage(String text) async {
    ...
    setState(() {
      _isSending = true;
      _suggestions.clear(); // clear existing suggestions
    });
    ...
    try {
      final resp = await http.post(
        Uri.parse('https://axilon-mini-be-e5732e59dadc.herokuapp.com/api/chats/messages'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ${authProvider.token}',
        },
      ...
      if (resp.statusCode == 200) {
        ...
        data = {'message': 'Failed to get reply', 'suggestions': []};
      }
    } catch (_) {
      data = {'message': 'Error sending message', 'suggestions': []};
    }
    // Show assistant reply and suggestions
    setState(() {
      _suggestions = List<String>.from(data['suggestions'] ?? []);
    });
    await _addAssistantReply(data['message'] ?? "");
  ...
  Future<void> _handleTaskCase(String text) async {
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    try {
      final resp = await http.post(
        Uri.parse('https://axilon-mini-be-e5732e59dadc.herokuapp.com/api/tasks/taskFromChat'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ${authProvider.token}',
        },
      ...
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        await _addAssistantReply(data['message'] ?? "Task created!");
      }
    } catch (_) {}
  }
  ...
  Future<void> _handleChatCase(String text) async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        await _addAssistantReply(data['message'] ?? "Chat updated!");
      }
    } catch (_) {}
  }
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            if (_isSending)
              Positioned(left: 16, bottom: 70, child: /* typing indicator */ Container()),
